...
# BitcoinFinder Web Admin

## Дефолтные креденшелы для входа в веб-интерфейс
...
## Как сменить логин/пароль

1. Через веб-интерфейс: вкладка "Настройки" (или через API `/api/auth/config` и `/api/auth/change-password`).
2. Можно вручную отредактировать файл `admin_config.json` (пароль хранится в виде SHA256-хэша).

## Вход в интерфейс

- Откройте [http://localhost:5000/login.html](http://localhost:5000/login.html)
